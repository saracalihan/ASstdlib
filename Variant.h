...
Variant variant_add(Variant a, Variant b){
    if(a.type != b.type) {
        printf( "[PLUS OPERATOR]: Can not add %s and %s\n", variant_to_string(&a), variant_to_string(&b));
        exit(1);
    }
    ...
    if (a.type == TYPE_INT) {
        ...
        printf( "[PLUS OPERATOR]: This type not implemented yet! Type number: %ul\n", a.type);
        exit(1);
    }
...
Variant variant_sub(Variant a, Variant b){
    if(a.type != b.type) {
             printf( "[SUBTRACTION OPERATOR]: Can not subtract %s and %s\n", variant_to_string(&a), variant_to_string(&b));
        exit(1);
    }
    ...
    if (a.type == TYPE_INT) {
        ...
        printf( "[SUBTRACTION OPERATOR]: Can not subtract string types\n");
        exit(1);
    } else if (a.type == TYPE_BOOL) {
        ...
        printf( "[SUBTRACTION OPERATOR]: This type not implemented yet! Type number: %ul\n", a.type);
        exit(1);
    }
...
Variant variant_mult(Variant a, Variant b){
    if(a.type != b.type) {
        printf( "[MULTIPLY OPERATOR]: Can not multiply %s and %s\n", variant_to_string(&a), variant_to_string(&b));
        exit(1);
    }
    ...
    if (a.type == TYPE_INT) {
        ...
        printf( "[MULTIPLY OPERATOR]: Only intagers can multiply!\n");
        exit(1);
    }
    return result;
}
Variant variant_div(Variant a, Variant b){
        if(a.type != b.type) {
        printf( "[DIVIDE OPERATOR]: Can not divide %s and %s\n", variant_to_string(&a), variant_to_string(&b));
    ...
    if (a.type == TYPE_INT) {
        ...
        printf( "[DIVIDE OPERATOR]: Only intagers can divide!\n");
        exit(1);
    }
    return result;
}
...
bool variant_is_truthy(const Variant a){
    ...
    switch (a.type){
        ...
        case TYPE_INT:
            ...
            break;
        case TYPE_NULL:
            ret = false; // null is always falsy
            break;
        default:
            printf("[TRUTHY OPERATOR] Not implemented yet!\n");
            exit(1);
    }
    return ret;
}
...
bool variant_and(const Variant a, const Variant b){
    if(a.type != b.type){
        printf("[LOGIC OPERATOR] Can not use && operator with %s and %s!\n", variant_to_string(&a), variant_to_string(&b));
        exit(1);
    }
...
bool variant_or(const Variant a, const Variant b){
    if(a.type != b.type){
        printf("[LOGIC OPERATOR] Can not use || with %s and %s!\n", variant_to_string(&a), variant_to_string(&b));
        exit(1);
    }
...
bool variant_gt(const Variant a, const Variant b){
    ...
    if(a.type == TYPE_INT){
        ...
        printf("[LOGIC OPERATOR] Can use > operator with %s and %s!\n", variant_to_string(&a), variant_to_string(&b));
        exit(1);
    }
}
...
bool variant_not(const Variant a){
    if(a.type != TYPE_STRING){
        printf("[NOT OPERATOR] Can get oposite of %s!\n", variant_to_string(&a));
        exit(1);
    }
